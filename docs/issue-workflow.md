...
7. **Code review** — spawn a review sub-agent via `claude-pool start`, wait for it
...
- **Unsubmitted prompts** (~25%): if 0% context after dispatch, send `claude-pool key <id> Enter`
- **Always `wait <id>`**, not bare `wait` (returns stale results)
- **`capture` empty for busy sessions**: read `~/.claude-pool/pools/*/slots/N/raw.log` directly
